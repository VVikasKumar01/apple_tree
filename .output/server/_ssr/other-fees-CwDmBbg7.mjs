...
import { u as useAcademicYear, C as CLASS_OPTIONS } from "./constants-C7cxSjft.mjs";
import { u as useAuth } from "./router-BTkK5XoE.mjs";
import { t as toast } from "../_libs/sonner.mjs";
import { l as Upload, D as Download, e as Search } from "../_libs/lucide-react.mjs";
...
function OtherFeesPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data: students
      } = await supabase.from("students").select("id,admission_number,student_name,class_grade").eq("academic_year", year).eq("school", school);
      const {
        data: fees
      } = await supabase.from("other_fees").select("*").eq("academic_year", year).eq("school", school);
