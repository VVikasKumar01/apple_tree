...
import { D as Dialog, a as DialogContent, b as DialogHeader, c as DialogTitle } from "./dialog-CZ3oge5o.mjs";
...
import { u as useAcademicYear, C as CLASS_OPTIONS, G as GENDER_OPTIONS, i as inr } from "./constants-C7cxSjft.mjs";
import { u as useAuth } from "./router-BTkK5XoE.mjs";
import { l as Upload, D as Download, e as Search, m as Save } from "../_libs/lucide-react.mjs";
...
function TuitionPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data: students
      } = await supabase.from("students").select("id,admission_number,student_name,class_grade,section,gender").eq("academic_year", year).eq("school", school);
      const {
        data: fees
      } = await supabase.from("tuition_fees").select("*").eq("academic_year", year).eq("school", school);
